                                 ...
                                 uint16_t      frame_size)
{
    ...
    while (len > 0)
    {
        ...
        // power_1 = adapt_rate / (power + 10)
        v_pwr = __riscv_vfadd_vf_f32m8(v_pwr, 10.0f, vl);

        // NOTE: might be possible to somehow remove vfrdiv using vfrec7
            ...
            spx_word16_t *mem)
{
    ...
    while (len > 0)
    {
        ...
        // NOTE: it shifs the elements to the right and drops the 
        // last element in the last position
        vfloat32m4_t v_prev = __riscv_vfslide1up_vf_f32m4(v_in, _mem, vl);

        _mem = _in[vl - 1];

        vfloat32m4_t v_out = __riscv_vfnmsac_vf_f32m4(v_in, _preemph, v_prev, vl);
