...
{
  return riscv_cfft_init_f32(p_instance, fft_length);
}
...
{
  riscv_cfft_f32(p_instance, p_buf, ifftFlag, bitReverseFlagR);
}

ee_status_t th_rfft_init_f32(ee_rfft_f32_t *p_instance, int fft_length)
{
  return riscv_rfft_init_f32(p_instance, fft_length);
}
...
{
  riscv_rfft_f32(p_instance, p_in, p_out, ifftFlag);
}
...
{
  riscv_absmax_f32(p_in, len, p_max, p_index);
}

void th_cmplx_mult_cmplx_f32(
  const ee_f32_t *p_a, const ee_f32_t *p_b, ee_f32_t *p_c, uint32_t len
)
{
  riscv_cmplx_mult_cmplx_f32(p_a, p_b, p_c, len);
}

void th_cmplx_conj_f32(const ee_f32_t *p_a, ee_f32_t *p_c, uint32_t len)
{
  riscv_cmplx_conj_f32(p_a, p_c, len);
}

void th_cmplx_dot_prod_f32(
  const ee_f32_t *p_a,
  const ee_f32_t *p_b,
  uint32_t len,
  ee_f32_t *p_r,
