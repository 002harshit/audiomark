...
{
    return v_riscv_cfft_init_f32(p_instance, fft_length);
}
...
{
    v_riscv_cfft_f32(p_instance, p_buf, ifftFlag, bitReverseFlagR);
}

ee_status_t
th_rfft_init_f32(ee_rfft_f32_t *p_instance, int fft_length)
{
    return v_riscv_rfft_init_f32(p_instance, fft_length);
}
...
th_rfft_f32(ee_rfft_f32_t *p_instance,
            ...
            uint8_t        ifftFlag)
{
    v_riscv_rfft_f32(p_instance, p_in, p_out, ifftFlag);
}
...
th_absmax_f32(const ee_f32_t *p_in,
              ...
              uint32_t       *p_index)
{
    v_riscv_absmax_f32(p_in, len, p_max, p_index);
}
...
th_cmplx_mult_cmplx_f32(const ee_f32_t *p_a,
                        ...
                        uint32_t        len)
{
    v_riscv_cmplx_mult_cmplx_f32(p_a, p_b, p_c, len);
}

void
th_cmplx_conj_f32(const ee_f32_t *p_a, ee_f32_t *p_c, uint32_t len)
{
    v_riscv_cmplx_conj_f32(p_a, p_c, len);
}
...
th_nn_classify(const int8_t in_data[490], int8_t out_data[12])
{
    ...
    return v_riscv_nn_classify(in_data, out_data);
}
