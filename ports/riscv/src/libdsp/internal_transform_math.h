...
static inline void get_cos_sin_values_f16 (int32_t ia, uint32_t m,
        float16_t *c, float16_t *s)
{
    const float16_t *ptr = &riscv_dsp_cos_table_f16[NDS32_COS_TABLE_OFFSET];
    TRANSFORM_MATH_GET_CS_VAL;
}
#endif
...
#if defined (__riscv_zfh)
extern const float16_t riscv_dsp_recip_table_f16[RECIP_TABLE_SIZE];
#endif
...
#if defined (__riscv_zfh)
static inline int32_t riscv_dsp_bit_reversal_f16(float16_t *src, uint32_t m)
{
    float16_t temp;
    uint32_t n = (1 << m);
    float16_t *const f1 = src + (2 * n) - 1;
    TRANSFORM_MATH_BIT_REV;
    return 0;
}
#endif
